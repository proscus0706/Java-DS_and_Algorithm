...
public class TwoDimensionalArray {
		...
		try {
			...
			for(int row = 0; row < arr.length; row++) {
				for(int col = 0; col < arr[0].length; col++) {
					...
					//굳이 말로 표현하자면.. 
					//arr[0].length가 햇갈릴 수 있다.
					//자바에서 배열은 '크기'를 정해 놓기때문에
					//arr[row][col], arr[col]  
					//아무 column이나 잡으면 column의 길이를 구할 수 있다. 
				}
				System.out.println();
			}
		} 
	...
	public void deleteValueFromArry(int deleteValueFromThisRow, int deleteValueFromThisCol) {
